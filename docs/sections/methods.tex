...
Although, during the research process certain empirical findings were discovered, hence the cores have been reduced to three
to supplement RAM use of the simulating device.
...
For each integrator, three solutions are computed:
\begin{itemize}
    \item \textbf{Baseline resolution:} $N = 10^5$ steps with $\Delta t = 10^{-1}$.
    \item \textbf{Medium resolution:} $N = 10^6$ steps with $\Delta t = 10^{-2}$.
    \item \textbf{High Resolution:} $N = 10^7$ steps with $\Delta t = 10^{-3}$.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textit{Convergence plots}: an overlay of trajectories at $\Delta t = 10^{-1}$, $\Delta t = 10^{-2}$ and
    $\Delta t = 10^{-3}$,
